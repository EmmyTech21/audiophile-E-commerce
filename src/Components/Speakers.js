...
export default function Speakers() {
  return (
    ...
    <div className='bg-[#010101] text-center text-white h-40'> 
        <h1 className='TEXT-4Xl py-20 font-manrope'>SPEAKERS</h1>
      </div>
    <div className='m-16'>
    
      {speaker.map((speakers,index)=>{
      return  <div className={`flex font-manrope flex-col gap-10 ${index === 1? 'lg:flex-row-reverse' : 'lg:flex-row'} items-center mb-16`}>
        <div className="flex justify-center items-center w-1/2 h-auto bg-[#f1f1f1]">
    ...
    <div className="w-full text-center  lg:text-left mt-10 md:w-1/2 p-4">
      ...
      <Link  to={speakers.href}className="px-8 py-4 inline-block mt-8 bg-[#d87d4A]  hover:bg-[#fbaf85]" >{speakers.link}</Link>
    </div>
  </div>
