...
import { headphones } from '../HeadData'; 
import ReuseService from '../ReuseService';

export default function ProductDetail() {
  const { id } = useParams();
  const product = headphones.find((product) => product.id === parseInt(id));
  ...
  return (
    ...
    <div className='flex rounded md:w-1/2 flex-col gap-4 mb-4'>
    <img className='rounded'  src={product.images3} alt='headphone'/>
    <img className='rounded'  src={product.images1} alt='headphone'/>
    </div>
    <img className='md:w-1/2 rounded sm:w-full' src={product.images2} alt='headphone'/>
    </div>
